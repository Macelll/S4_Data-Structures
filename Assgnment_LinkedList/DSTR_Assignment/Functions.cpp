...
void displayTutor() {
	...
	while (true) {
		if (confirm == 1 || confirm == 2) {
			if (confirm == 2) {					//if user wants to go to previous page, the current pointer must be -10 to return to the previous page
				/*if (current == NULL) {
					current = tail;
				}*/
				current = current->prev->prev->prev->prev->prev->prev->prev->prev->prev;
				count -= 10;
				/*if (current == NULL) {
					current = head;
				}*/
				...
				/*try {
					current = current->prev->prev->prev->prev->prev->prev->prev->prev->prev->prev;
					count -= 10;
				}
...
void modifyTutor() {
	...
	if (found) {
		...
		cout << "\nNew Tutor Phone       : ";   cin >> current->phone;				cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
		cout <<   "New Tutor Address     : ";   getline(cin, current->address);
